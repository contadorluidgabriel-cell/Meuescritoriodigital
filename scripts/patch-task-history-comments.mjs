...
export function applyTaskHistoryCommentsPatch(root) {
  ...
  if (!source.includes('MED_TASK_HISTORY_COMMENTS_V1')) {
    ...
    source = replaceRequired(
      ...
      "<Field label=\"Descrição\" full><textarea value={editing.descricao} onChange={event => setField('descricao', event.target.value)} /></Field><Field label=\"Observação\" full><textarea value={editing.observacao || ''} onChange={event => setField('observacao', event.target.value)} placeholder=\"Registre contexto, resultado, protocolo ou detalhe importante da execução.\" /></Field>{editing.id ? <Field label=\"Comentários\" full><div className=\"task-comments-editor\">{taskComments(editing).map(comment => <article key={comment.id || comment.criadoEm}><div><strong>{comment.autor || 'Usuário'}</strong><small>{comment.criadoEm ? new Date(comment.criadoEm).toLocaleString('pt-BR') : ''}</small></div><p>{comment.texto}</p><button type=\"button\" onClick={() => { removeTaskComment(editing.id, comment.id); setEditing(current => ({ ...current, comentarios: taskComments(current).filter(item => String(item.id) !== String(comment.id)) })) }}>Remover</button></article>)}<div className=\"task-comment-compose\"><input value={commentDraft} onChange={event => setCommentDraft(event.target.value)} placeholder=\"Adicionar comentário…\" /><button type=\"button\" disabled={!commentDraft.trim()} onClick={() => { addTaskComment(editing.id); setEditing(current => ({ ...current, comentarios: [...taskComments(current), { id: 'local_' + Date.now(), texto: commentDraft.trim(), criadoEm: new Date().toISOString(), autor: taskCommentAuthor() }] })); setCommentDraft('') }}>Comentar</button></div></div></Field> : null}",
