...
export function applyAccessRoutesPatch(root) {
  ...
  source = replaceOrFail(source,
    ...
    "      {view === 'clientes' ? (access?.membership?.role === 'partner' || (access?.membership?.role === 'collaborator' && !access?.membership?.permissions?.manage_clients) || (access?.membership?.role === 'admin' && access?.membership?.permissions?.access_v2 === true && String(access?.membership?.user_id || '') !== String(access?.workspace?.owner_user_id || '') && !access?.membership?.permissions?.manage_clients) ? <ClientDirectoryReadOnly office={office} access={access} onOpenTasks={openTasksForClient} onOpenProcesses={openProcessesForClient} /> : <ClientsReact office={office} update={update} sync={sync} access={access} onRefresh={refreshWorkspace} onOpenTasks={openTasksForClient} onOpenProcesses={openProcessesForClient} onOpenFinance={openFinanceForClient} initialClientId={clientTarget.id} openClientRequest={clientTarget.request} />) : null}",
