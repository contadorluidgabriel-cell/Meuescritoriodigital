...
test('edição salva status novo e não conserva data de uma etapa reaberta', () => {
  ...
  assert.match(ui, /const unfinishedStep = processActionState\(normalized\)\.step/)
  assert.match(ui, /isDone\(normalized\.status\) && unfinishedStep\) Object\.assign\(normalized, setCurrentProcessStep\(normalized, unfinishedStep\.id\)\.process\)/)
