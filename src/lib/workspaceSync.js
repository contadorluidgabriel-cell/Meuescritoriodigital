...
const officeNames = [
  'clients', 'linkedCompanies', 'partners', 'tasks', 'taskTemplates', 'processes', 'obligations', 'processModels', 'finance',
  'financeAccounts', 'financePayables', 'financeMovements', 'financeCategories', 'financeRecurrences', 'financeClosings', 'financeCollectionEvents', 'financeConfig',
...
function allowedNames(access = {}) {
  ...
  if (receivables) { result.add('finance'); result.add('financeCollectionEvents') }
  if (permissions.finance_payables) result.add('financePayables')
  if (permissions.finance_cash) { result.add('financeAccounts'); result.add('financeMovements') }
  return result
}
...
export function buildOfficePatch(before = {}, after = {}, access = {}) {
  ...
  for (const name of officeNames) {
    ...
    if (arrayNames.has(name)) {
      ...
      patch[name] = { replace: clone(after[name]) }
    }
  }
  return patch
}
