...
export function applyTaskHistoryCommentsPatch(root) {
  ...
  if (!source.includes('task-history-panel')) {
    ...
    source = replaceRequired(
      source,
      "<section className=\"react-module-card\"><div className=\"task-filters\">",
      "<section className=\"react-module-card\"><div className=\"task-history-toolbar\"><button type=\"button\" className=\"task-history-link\" onClick={() => setHistoryOpen(current => !current)}>Histórico de tarefas {completedCount ? `(${completedCount})` : ''}</button></div><div className=\"task-filters\">",
      'history access',
      taskPath,
    )
